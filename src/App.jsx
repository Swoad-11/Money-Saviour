...
import { LayoutDashboard, PieChart, Target, History, RefreshCw } from 'lucide-react';
...
const DEFAULT_EXPENSES = [
  ...
  { id: 3, name: 'Transport', amount: '', recurring: false, frequency: 'monthly' },
  { id: 4, name: 'Utilities', amount: '', recurring: true, frequency: 'monthly' },
];
...
export default function App() {
  ...
  const [expenses, setExpenses] = useLocalStorage('ms_expenses', DEFAULT_EXPENSES);
  ...
  return (
    <div className={dark ? 'dark' : ''}>
      <div className="min-h-screen bg-gray-50 dark:bg-zinc-950 transition-colors duration-300">
        <Navbar dark={dark} setDark={setDark} currency={currency} setCurrency={setCurrency} />
        ...
        <main className="max-w-3xl mx-auto px-4 py-6">
          ...
          {activeTab === 'budget' && (
            <BudgetTab
              ...
              savingPct={savingPct} setSavingPct={setSavingPct}
              history={history} setHistory={setHistory}
            />
          )}
          {activeTab === 'overview' && (
            <OverviewTab income={income} expenses={expenses} savingPct={savingPct} currency={currency} />
          )}
          {activeTab === 'recurring' && (
            <RecurringTab expenses={expenses} currency={currency} income={income} />
          )}
          {activeTab === 'goals' && (
            <GoalsTab currency={currency} monthlySaving={monthlySaving} />
          )}
          {activeTab === 'history' && (
            <HistoryTab history={history} setHistory={setHistory} currency={currency} />
          )}
        </main>
