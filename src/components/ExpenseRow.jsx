...
export default function ExpenseRow({ expense, onChange, onDelete, symbol }) {
  ...
  return (
    <div className={`rounded-xl border transition-colors ${expense.recurring ? 'border-brand-100 dark:border-brand-800/40 bg-brand-50/40 dark:bg-brand-900/10' : 'border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-800/30'} p-3`}>
      <div className="flex items-center gap-2">
        ...
        <div className="relative flex items-center">
          <span className="absolute left-3 text-xs text-zinc-400 dark:text-zinc-500 pointer-events-none">{symbol}</span>
          <input
            type="number"
        ...
        <button
          onClick={() => { update('recurring', !expense.recurring); setShowRecurring(!expense.recurring); }}
        ...
        >
          <ChevronDown size={14} className={`transition-transform ${showRecurring ? 'rotate-180' : ''}`} />
        </button>

        <button onClick={onDelete} className="p-1.5 rounded-lg text-zinc-300 hover:text-red-400 transition-colors">
          <Trash2 size={14} />
        </button>
      </div>
